...
//获取从x到y之间的随机数
#define RANDSTR(x,y)  ((char)(rand()%(y-x)+x)) 
#define PATTERNLEN 40

/* define function here */
void usage(char *s);
void transdata(int fd1,int fd2);
void closeallfd();
void makelog(char *buffer,int length);  // 写日志 
...
void replace_flag(char *str); //替换

/* define GLOBAL varible here */
extern int errno;
FILE *fp; 
regex_t reg; //编译的正则表达式 
int regflag=0;


void replace_flag(char *str){
    if(!regflag){
        return ;
    }
    ...
    srand(time(0)/(60*3));  //三分钟之内不会变
    // strncpy(realstr,str,len);
    status = regexec(&reg,str,nmatch,pmatch,0);
    if(status == REG_NOMATCH){
        return ;
    }else if(status == 0){
        for(i=pmatch[0].rm_so;i<pmatch[0].rm_eo;i++){   
            str[i] = RANDSTR(48,126);
        }
        // printf("find str!");
    }
}
...
int main(int argc,char **argv)
{
    ...
    while(*p)
    {
        ...
        if(strcmp(*p,"-r")==0){
            if(testifisvalue(*(p+1))==1){
                length = (strlen(*(p+1))>PATTERNLEN-1)?PATTERNLEN-1:strlen(*(p+1));
                strncpy(pattern,*(++p),length);   
                if(strlen(pattern)==0){
                    printf("[ERROR]:must supply a regex pattern\r\n");
                    exit(0);
                }

                rnt = regcomp(&reg,pattern,cflags);
                if(rnt){
                    printf("[ERROR]:the pattern is error!\r\n");
                    exit(0);
                }
                regflag = 1;
    ...
    signal(SIGINT,&closeallfd);   //但产生ctrl+c 信号的时候,关闭文件指针
...
void usage(char *s)
{
    ...
    printf(" -r: a regex pattern to replace data come from address2,Example1:flag\\\\{[a-zA-Z0-9_]{4}\\\\}?\r\n Example:flag\\\\{[a-zA-Z0-9_]+\\\\}?\r\n");
    closeallfd();
}
...
void transdata(int fd1,int fd2)
{
    ...
    while(1)
    {
        ...
        {
            /* 不能超过MAXSIZE-total read1,不然send_out1会溢出 */
        ...
        {   
            ...
            {   
                ...
                replace_flag(send_out2); //修改从fd2,传向fd1的数据 
